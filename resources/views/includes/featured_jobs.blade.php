        ...
        <div class="jobint">
          ...
          <div class="row">
            ...
            <div class="col-md-7 col-sm-7">
              ...
              <div class="company"><a href="{{route('company.detail', $company->slug)}}" title="{{$company->name}}">{{$company->name}}  
              &nbsp;&nbsp; {{__('TJM')}}: {{$featuredJob->salary_currency}} {{$featuredJob->salary_from}}-{{$featuredJob->salary_to}}
              </a></div>

              <div class="jobloc">

              <label class="fulltime" title="{{$featuredJob->getJobType('job_type')}}">{{$featuredJob->getJobType('job_type')}}</label> - <span>{{$featuredJob->getCity('city')}}</span>
               <br><br> <label class="duration" title="{{__('Duration')}}"> {{__('Duration')}}</label> - <span> {{ date_format($featuredJob->expiry_date,'Y-m-d')}}</span>
