...
@include('includes.inner_page_title', ['page_title'=>__('Join Choose')]) 
        ...
        <div class="userccount">
          <div class="userbtns">
            ...
            // $c_or_e = old('candidate_or_employer', 'candidate');
                    $c_or_e = isset($type) ? ($type == 'freelancer' ? 'candidate' : 'employer') : 'candidate';
                    
                   ?>
                 <li class="{{($c_or_e == 'candidate')? 'active w-100':'hide'}}" style='{{($c_or_e == 'candidate') ? "width:100%;" : ''}}' ><a data-toggle="tab" href="#candidate" aria-expanded="true">{{__('Freelance')}}</a></li>
                 <li class="{{($c_or_e == 'employer')? 'active w-100':'hide'}}" style='{{($c_or_e == 'employer') ? "width:100%;" : ''}}'  ><a data-toggle="tab" href="#employer" aria-expanded="false">{{__('Employer')}}</a></li>  
             </ul>
          </div>
          <div class="tab-content">
            <div id="candidate" class="formpanel tab-pane fade {{($c_or_e == 'candidate')? 'active in':''}}">
              <form class="form-horizontal" method="POST" action="{{ route('register') }}">
                ...
                <!-- <div class="formrow{{ $errors->has('g-recaptcha-response') ? ' has-error' : '' }}"> {!! NoCaptcha::display() !!}
          ...
          <!-- sign up form -->
          <div class="newuser"><i class="fa fa-user" aria-hidden="true"></i> {{__('Have Account')}}? <a href="{{ $c_or_e == 'candidate' ? route('login') : route('recruiterlogin')}}">{{__('Sign in')}}</a></div>
          <!-- sign up form end--> 
